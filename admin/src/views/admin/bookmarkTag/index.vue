...
function getList() {
  loading.value = true;
  listBookmarkTag(queryParams.value).then(response => {
    bookmarkTagList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("bookmarkTagRef");
}
  ...
  proxy.$refs["bookmarkTagRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        ...
        addBookmarkTag(form.value).then(response => {
...
function handleExport() {
  proxy.download('admin/bookmarkTag/export', {
    ...queryParams.value
  }, `bookmarkTag_${new Date().getTime()}.xlsx`)
}
